...
function render ( pkg, currentState ) {
  ...
  const indent = '           ' + emoji( '   ' );
  const flags = `${ pkg.devDependency ? '--dev ' : '' }`;
  const upgradeCommand = `composer require ${ flags }${ packageName }@${ pkg.latest }`;
  ...
  const status = _( [
    ...
    pkg.unused ? [
      ...
      indent + `To remove this package: ${ chalk.green( `composer remove ${ pkg.devDependency ? '--dev ' : '' }${ packageName }` ) }`
    ] : '',
    ...
    pkg.regError ? chalk.bgRed.white.bold( emoji( ' :no_entry: ' ) + ' COMPOSER ERR! ' ) + ' ' + chalk.red( pkg.regError ) : ''
  ] )
    .flatten()
    .compact()
    .valueOf();
...
function outputConsole ( currentState ) {
  ...
  if ( rows.length ) {
    ...
    console.log( `Use ${ chalk.green( `composer-check -${ currentState.get( 'global' ) ? 'g' : '' }u` ) } for interactive update.` );
    process.exitCode = 1;
  } else {
