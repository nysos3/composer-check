...
function render(pkg, currentState) {
    ...
    const upgradeCommand = `composer install ${flags} ${packageName}@${pkg.latest}`;
    ...
    const status = _([
        ...
        pkg.unused ? [
            ...
            indent + `To remove this package: ${chalk.green(`composer uninstall --save${pkg.devDependency ? '-dev' : ''} ${packageName}`)}`
        ] : '',
