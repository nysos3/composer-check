...
function updateAll(currentState) {
    ...
    return installPackages(saveDependencies, currentState)
        ...
        .then(currentState => {
            console.log('');
            console.log(chalk.green(`[composer-check] Update complete!`));
            console.log(chalk.green('[composer-check] ' + updatedPackages));
            console.log(chalk.green(`[composer-check] You should re-run your tests to make sure everything works with the updates.`));
            return currentState;
        });
}
