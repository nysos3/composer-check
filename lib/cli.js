...
const composerCheck = require( './index' );
...
const cli = meow( {
  help: `
        ...
        Examples
          $ composer-check           # See what can be updated, what isn't being used.
          $ composer-check ../foo    # Check another path.
          $ composer-check -gu       # Update globally installed modules by picking which ones to upgrade.
    `},
  {
    alias: {
      u: 'update',
...
const options = {
  ...
  installer: process.env.COMPOSER_CHECK_INSTALLER || 'auto',
...
Promise.resolve()
  ...
  .then( async ( installer ) => {
    ...
    const ret = await composerCheck( options );
    return ret
  } )
  .then( currentState => {
...
const SUPPORTED_INSTALLERS = ['composer'];
